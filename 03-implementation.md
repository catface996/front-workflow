...
```bash
# 检查常见的布局文件位置
ls -la src/layouts/ src/components/Layout* src/app/layout.* 2>/dev/null

# React 项目
find src -name "*Layout*" -o -name "*layout*" | head -20

# Vue 项目
find src -name "*.vue" | xargs grep -l "<header>\|<aside>\|<main>" 2>/dev/null | head -10

# Next.js 项目
ls -la src/app/layout.tsx app/layout.tsx 2>/dev/null
```
...
| 检测项 | 检测文件 | 状态 |
|--------|---------|------|
| 布局组件 | `src/layouts/`, `src/components/Layout*` | ⏳ |
| 页面结构 | Header, Sidebar, Main, Footer | ⏳ |
| 响应式处理 | 媒体查询, 断点变量 | ⏳ |
| 布局样式 | CSS/SCSS/CSS Modules | ⏳ |
...
│  ┌─────────────────────────────────────────────────────┐   │
│  │ [ ] 布局结构是否与设计稿一致？                        │   │
│  │ [ ] Header 高度是否正确？ (现有: 60px)               │   │
│  │ [ ] Sidebar 宽度是否正确？ (现有: 240px)             │   │
│  │ [ ] 响应式断点是否符合需求？                          │   │
│  │ [ ] 是否需要新增其他布局？(如 AuthLayout)            │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  请确认:                                                    │
│  [ ] 现有骨架符合设计稿，直接使用                            │
│  [ ] 现有骨架需要调整: _______________                       │
│  [ ] 需要新增布局组件: _______________                       │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
⚠️ 与设计稿对比，请确认:
1. Header 高度: 现有 60px，设计稿 64px → 需要调整？
2. Sidebar 宽度: 现有 240px，设计稿 256px → 需要调整？
3. 响应式: 现有 <768px 隐藏 Sidebar，是否符合设计？
4. 是否需要新增 AuthLayout（登录/注册页布局）？

---
请确认是否可以使用现有骨架，或指出需要调整的地方。
```
...
│  ═══════════════════════════════════════════════════════   │
│  请确认设计稿的页面结构:                                      │
│  [ ] 标准后台布局 (Header + Sidebar + Main + Footer)        │
│  [ ] 顶部导航布局 (Header + Main + Footer，无Sidebar)       │
│  [ ] 双侧边栏布局 (Header + 左Sidebar + Main + 右Sidebar)   │
│  [ ] 简洁布局 (仅 Main，无Header/Sidebar)                   │
│  [ ] 其他: _______________                                  │
│                                                             │
│  ═══════════════════════════════════════════════════════   │
│  2️⃣ Header 配置                                             │
│  ═══════════════════════════════════════════════════════   │
│  [ ] 高度: ___ px (常见: 56/60/64px)                        │
│  [ ] 定位: [ ] fixed吸顶 / [ ] static跟随滚动               │
│  [ ] 内容: [ ] Logo [ ] 导航菜单 [ ] 搜索框 [ ] 用户信息    │
│                                                             │
│  ═══════════════════════════════════════════════════════   │
│  3️⃣ Sidebar 配置                                            │
│  ═══════════════════════════════════════════════════════   │
│  [ ] 宽度: ___ px (常见: 200/240/256px)                     │
│  [ ] 收起宽度: ___ px (常见: 60/64/80px)                    │
│  [ ] 定位: [ ] fixed固定 / [ ] sticky粘性                   │
│  [ ] 功能: [ ] 可折叠 [ ] 响应式隐藏 [ ] 抽屉模式           │
│                                                             │
│  ═══════════════════════════════════════════════════════   │
│  4️⃣ Main 配置                                               │
│  ═══════════════════════════════════════════════════════   │
│  [ ] 最大宽度: ___ px (常见: 1200/1400/无限制)              │
│  [ ] 内边距: ___ px (常见: 16/20/24px)                      │
│  [ ] 内容居中: [ ] 是 [ ] 否                                │
│                                                             │
│  ═══════════════════════════════════════════════════════   │
│  5️⃣ 响应式断点                                              │
│  ═══════════════════════════════════════════════════════   │
│  [ ] 使用技术栈默认断点 (Tailwind/组件库)                    │
│  [ ] 自定义断点:                                            │
│      移动端: < ___ px                                       │
│      平板: ___ - ___ px                                     │
│      桌面: > ___ px                                         │
...
1️⃣ 页面整体结构
   根据设计稿，请选择布局类型:
   1. 标准后台布局 (Header + Sidebar + Main + Footer)
   2. 顶部导航布局 (Header + Main + Footer)
   3. 双侧边栏布局
   4. 简洁布局 (仅Main)
   请选择: ___
...
---
请按以下格式回复 (示例):
1, 64px/是/Logo+导航+用户, 240px/60px/是/抽屉, 1200px/24px, 768/1024
```
...
| 断点 | 宽度 | 布局变化 | 确认 |
|------|------|---------|------|
| 移动端 | <768px | Sidebar隐藏，抽屉模式 | ✅ |
| 平板 | 768-1023px | Sidebar收起60px | ✅ |
| 桌面 | ≥1024px | 完整布局 | ✅ |
